<x-layouts.admin>
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800">Crear Nuevo Post</h2>
            <p class="text-gray-600 mt-2">Crea contenido para el blog de Puente Papel</p>
        </div>
    ...
    <div class="bg-pink-50 px-8 py-6 rounded-2xl max-w-2xl mx-auto">
        <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" id="blog-form" novalidate>
            @csrf
            ...
            <div class="space-y-6">
                ...
                {{-- Editor Markdown --}}
                <div>
                    <dile-editor name="content" label="Contenido del Post *" value="{{ old('content') }}" language="es"
                        viewSelected="editor"></dile-editor>
                    @error('content')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
